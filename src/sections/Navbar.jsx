...
const Navbar = () => {
  ...
  const [showNav, setShowNav] = useState(true);
   useEffect(() => {
    document.title = "Yathartha Shrestha"; 
  }, []);
  useGSAP(() => {
    // Start hidden
 ...
 iconTl.current = gsap.timeline.apply({paused: true }).to(topLineRef.current,{
  rotate:45,
  y: 3.5,
  duration: 0.3,
  ease: "Power2.inou",
    ...
    <>
      ...
      >
        <div className="flex flex-col text-5xl gap-y-2 md:text-6xl lg:text-8xl">
          {["home", "services", "about", "work", "content"].map((section, index) => (
      ...
      <div
        className="fixed z-50 flex-col flex items-center justify-center gap-1 transition-all duration-300 bg-black 
        rounded-full cursor-pointer w-14 h-14 md:w-20 md:h-20 top-4 right-10"
        onClick={toggleMenu}
        style={showNav ? { clipPath: "circle(50% at 50% 50%)" }
